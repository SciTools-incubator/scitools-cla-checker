...
def main():
    ...
    parser = argparse.ArgumentParser(
            description='Print/check the contributors list')
    parser.add_argument(
            'check', default=None, nargs='?',
            help=('[If defined] Check that the given name is in the '
                  'contributors list'))
    ...
    if args.check is None:
        ...
        if args.check not in signatures:
            print('{} is not in the list of contributors ({})'
                  ''.format(args.check, ', '.join(signatures)))
            sys.exit(1)
        else:
